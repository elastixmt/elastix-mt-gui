...
require_once 'apps/extensions/libs/paloSantoExtensions.class.php';
...
class paloControlPanelStatus extends paloInterfaceSSE
{
    ...
    private function _loadStaticDataFromDatabase()
    {
        ...
        if (!is_null($this->_domain)) {
        	$sqlcond[] = 'organization_domain = ?';
            $sqlparam[] = $this->_domain;
        }
        $recordset = $this->_db->fetchTable(
            'SELECT dial AS channel, tech, exten AS extension, voicemail '.
            'FROM extension '.
            ((count($sqlcond) > 0) ? 'WHERE '.implode(' AND ', $sqlcond).' ' : '').
            'ORDER BY dial',
            TRUE, $sqlparam);
        if (is_array($recordset)) foreach ($recordset as $tupla) {
            $phonestate = array(
                ...
                'mailbox'           =>  ($tupla['voicemail'] != 'novm') ? $tupla['extension'].'@'.$tupla['voicemail'] : NULL,
                'UrgMessages'       =>  0,
                'NewMessages'       =>  0,
                'OldMessages'       =>  0,
    ...
    private function _loadStaticDataFromDatabase_bak()
    {
        // Recoger todas las extensiones, con todas las tecnologías
    	$recordset = $this->_db->fetchTable(
    ...
    private function _loadAreaAssignments()
    {
         ...
         * modificarse cuando se porte esto a Elastix 3. */
        $sqlAreaExt =
